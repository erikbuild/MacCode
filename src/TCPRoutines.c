...
OSErr LowTCPCreateStream(StreamPtr *streamPtr,Ptr connectionBuffer,
			unsigned long connBufferLen,TCPNotifyProcPtr notifPtr, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPWaitForConnection(StreamPtr streamPtr,SInt8 timeout,ip_addr *remoteHost,
			tcp_port *remotePort,ip_addr *localHost,tcp_port *localPort,
			Boolean async,TCPiopb **returnBlock, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPOpenConnection(StreamPtr streamPtr, SInt8 timeout,ip_addr remoteHost,
			tcp_port remotePort,ip_addr *localHost,tcp_port *localPort, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPSendData(StreamPtr streamPtr, SInt8 timeout,Boolean push,Boolean urgent,
					Ptr wdsPtr,Boolean async,TCPiopb **returnBlock, GiveTimePtr giveTimePtr, bool* cancel)
{	
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPNoCopyRcv(StreamPtr streamPtr, SInt8 timeout,Boolean *urgent,Boolean *mark,
				Ptr rdsPtr,short numEntry,Boolean async,TCPiopb **returnBlock, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr	err = noErr;
	TCPiopb *pBlock;
...
OSErr LowTCPBfrReturn(StreamPtr streamPtr,Ptr rdsPtr, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPRecvData(StreamPtr streamPtr, SInt8 timeout,Boolean *urgent,Boolean *mark,
				Ptr rcvBuff,unsigned short *rcvLen,Boolean async,TCPiopb **returnBlock, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPClose(StreamPtr streamPtr, SInt8 timeout, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPAbort(StreamPtr streamPtr, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPStatus(StreamPtr streamPtr,TCPStatusPB *theStatus, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPRelease(StreamPtr streamPtr,Ptr *recvPtr,unsigned long *recvLen, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
OSErr LowTCPGlobalInfo(Ptr *tcpParam,Ptr *tcpStat, GiveTimePtr giveTimePtr, bool* cancel)
{
	OSErr err;
	TCPiopb *pBlock;
...
//OSErr GetMyIP(ip_addr *ipnum, GiveTimePtr giveTimePtr, bool* cancel)
//{
...
//	while (IPBlock->ioResult > 0) {
//		(*giveTimePtr)();
//
//		if (*cancel == true)
//			return -1;
//}
//	*ipnum = IPBlock->csParam.;
//	err = IPBlock->ioResult;
//	DisposePtr((Ptr)IPBlock);
//	return err;
//}
